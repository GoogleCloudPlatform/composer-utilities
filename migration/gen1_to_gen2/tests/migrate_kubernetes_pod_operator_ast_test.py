...
TEST_RESOURCES_DIR = os.path.join(os.path.dirname(TEST_DIR), "test_resources")


def test_find_problem_kubernetes_pod_operator() -> None:
    output = CheckKubernetesPodOperator().check_for_problem(
        os.path.join(TEST_RESOURCES_DIR, "kubernetes_pod_operator_airflow_1.py")
    )
...
def test_find_problem_kubernetes_pod_operator_no_problem() -> None:
    problem_operators_airflow_1_no_affinity = (
        CheckKubernetesPodOperator().check_for_problem(
            os.path.join(
                TEST_RESOURCES_DIR, "kubernetes_pod_operator_airflow_1_no_affinity.py"
            )
        )["nodes"]
    )
    problem_operators_airflow_1_gke = CheckKubernetesPodOperator().check_for_problem(
        os.path.join(TEST_RESOURCES_DIR, "gke_operator_airflow_1.py")
    )["nodes"]  # has affinity but is ok to have it
    problem_operators_hadoop = CheckKubernetesPodOperator().check_for_problem(
        os.path.join(TEST_RESOURCES_DIR, "hadoop_tutorial.py")
    )["nodes"]  # has nothing to do with k8s
    problem_operators_airflow_db = CheckKubernetesPodOperator().check_for_problem(
        os.path.join(TEST_RESOURCES_DIR, "airflow_db_cleanup.py")
    )["nodes"]  # has nothing to do with k8s but has multiple targets in an assign
