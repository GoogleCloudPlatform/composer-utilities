...
class AssignFinder(ast.NodeVisitor):
    def __init__(self) -> None:
        self.possible_kpo_nodes: list[ast.Assign] = []
...
class KPOFinder(ast.NodeVisitor):
    def __init__(self) -> None:
        ...
        self.kpo_attribute: bool = False

    # Binding to ast.AST means we can accept any node type
    def visit_value(self, node: A) -> None:
        print(type(node))
        self.generic_visit(node)
...
class CheckKubernetesPodOperator(DAGChecker):
    def table_title_text(self) -> dict:
        return {
    ...
    def check_for_problem(self, file: str) -> dict:
