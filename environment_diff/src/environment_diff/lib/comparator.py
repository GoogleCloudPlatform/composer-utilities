...
from environment_diff.lib.strategies.strategy import EnvironmentAttributeDiffer
...
class EnvironmentComparator:
    ...
    def output_diffs_as_json(self: T) -> None:
        with open("environment_diff.json", "w") as write_file:
            json_data: dict[str, list] = {"differences_detected": []}
            for diff in self.differences:
